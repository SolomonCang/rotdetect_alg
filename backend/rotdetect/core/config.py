...
class SearchConfig:
    ...
    def validate(self) -> None:
        if self.period_min <= 0 or self.period_max <= self.period_min:
            raise ValueError(
                "period range must satisfy 0 < period_min < period_max")
        if self.delta_p_min <= 0 or self.delta_p_max <= self.delta_p_min:
            raise ValueError(
                "delta_p range must satisfy 0 < delta_p_min < delta_p_max")
        ...
        if self.max_peaks < self.min_modes:
            raise ValueError(
                "max_peaks must be greater than or equal to min_modes")
